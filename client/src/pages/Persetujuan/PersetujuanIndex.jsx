...
export default function PersetujuanIndex() {
  ...
  const fetchDaftar = async () => {
    try {
      const res = await getData(`/daftarmbkm/kaprodi`, formSearch);
  ...
  useEffect(() => {
    setTimeout(() => {
      if (JSON.parse(localStorage.getItem("role")) !== "kaprodi") {
        navigate("/forbidden");
      }
      fetchProdi();
      fetchDaftar();
